...
export default function AdminDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const [articles, users, comments, categories] = await Promise.all(responses.map(r => r.json()));
      
      setStats({
        totalArticles: articles.count || articles.length,
        totalUsers: users.count || users.length,
        totalComments: comments.count || comments.length,
  ...
  useEffect(() => {
    ...
    fetchTabData();
  }, [activeTab, searchTerm, userSearchTerm, commentSearchTerm, fetchData,fetchStats]);
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <Link href={`/profile/${user.id}`} className="font-medium hover:text-blue-600">
                              {user.username}
                            </Link>
                            {user.is_staff && (
                  ...
                  {comments.map(comment => (
                    <div key={comment.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <Link href={`/profile/${comment.user.id}`} className="font-medium hover:text-blue-600">
